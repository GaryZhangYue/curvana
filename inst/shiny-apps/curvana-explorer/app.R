            ...
            shiny::conditionalPanel(
              ...
              shiny::div(style = "margin-top: 8px;",
                shiny::radioButtons(
                  ...
                  label = NULL,
                  choiceNames = list(
                  shiny::HTML("<strong>Bruker NanoScope/Veeco ASCII</strong> <a href='examples/input.example.nanoscopeASCII.txt' target='_blank' style='font-size: 0.85em; margin-left: 6px;'>(View example file)</a><br><span style='font-size: 0.9em; color: #6c757d;'>Data are column-separated with piezo displacement and cantilever deflection voltage stored in different columns for approach and retract segments. Typical column names include: Calc_Ramp_Ex_nm, Calc_Ramp_Rt_nm, Defl_V_Ex, Defl_V_Rt.</span>"),
                  shiny::HTML("<strong>JPK ASCII Format</strong> <a href='examples/input.example.JPKASCII.txt' target='_blank' style='font-size: 0.85em; margin-left: 6px;'>(View example file)</a><br><span style='font-size: 0.9em; color: #6c757d;'>JPK text export files contain approach (extend) and retract segments marked by segment headers. The function automatically detects segments and extracts sensitivity and spring constant values separately for each. Users are encouraged to export deflection data in Voltage. If force data is provided (indicated by 'N' unit), it will be converted back to Voltage using the associated sensitivity and spring constant values.</span>"),
                  shiny::HTML("<strong>Generic Column-Separated Format</strong> <a href='examples/input.example.veeco.txt' target='_blank' style='font-size: 0.85em; margin-left: 6px;'>(View example file)</a><br><span style='font-size: 0.9em; color: #6c757d;'>Generic format for column-separated AFM data files. Piezo displacement and cantilever deflection voltage are stored in separate columns. Files can contain approach segment only, retract segment only, or both segments.</span>")
                ),
                choiceValues = list(
